...
bool FMP3_RuntimeCodec::Decode(FEncodedAudioStruct EncodedData, FDecodedAudioStruct& DecodedData)
{
	...
	float* TempPCMData = static_cast<float*>(FMemory::Malloc(SoundInfo.samples * sizeof(float)));
	...
	for (size_t i = 0; i < SoundInfo.samples; ++i)
	{
		TempPCMData[i] = static_cast<float>(SoundInfo.buffer[i]) / 32768.0f;
	}
	DecodedData.PCMInfo.PCMData = FRuntimeBulkDataBuffer<float>(TempPCMData, SoundInfo.samples);
